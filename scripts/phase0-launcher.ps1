...
cd AI_Agents_Hub
python troptions_sovereign_orchestrator.py --exec "run phase 0 $Mode" 2>$null || python -c "
import sys
sys.path.append('.')
...
cd troptions-rails/contracts 2>$null
& forge build 2>$null || Write-Host "Forge not found or no local clone - use GH Codespaces or local install"
& forge test --match-contract "Test*" -vv 2>$null || Write-Host "Tests skipped (no local setup)"

# 4. Proofs (LPS-1)
Write-Host "`n[4/5] Proof Unification (LPS-1)..."
cd ../troptions-avalanche-sports 2>$null
npm run proof:build 2>$null || Write-Host "Proof build (run manually if in avalanche-sports: npm run proof:build && npm run proof:lps1)"
